...
@bot.message_handler(commands=['ru','ua'])
...
#@bot.message_handler(commands=['T1','T2','T3','T4','T5'])
...
def button(query):
    ...
    #print('query.data:', query.data, query.data.startswith('/T'))
    if query.data.startswith('/T'):
        command = query.data.upper()[1:]
        #print('button:%s [%s] %s' % (command, query.data, query.message.text))
...
@bot.message_handler(func=lambda message: message and message.text.startswith('/send') and True)
def send(message):
