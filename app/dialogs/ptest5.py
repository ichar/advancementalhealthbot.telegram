...
_SCORES = {
    'ru': [
        ...
        ('B', 'один раз в неделю',),
        ('C', '2-4 раза в неделю / половину времени',),
        ('D', 'почти постоянно)'), 
    ],
    'ua': [
        ('A', 'зовсім не вірно або траплялося одноразово',),
        ('B', 'один раз на тиждень',),
        ('C', '2-4 рази на тиждень / половину від усього часу',),
        ('D', 'майже постійно',),
    ],
}
...
_RESULTS = {
    'ru' : (
        ...
        (17, 'Обнаружены симптомы психотравмы, требующие психологического сопровождения специалистов травмофокусирующей направленности'), 
        (34, 'Обнаружены субклинически проявленные симптомы психотравмы (II звено)'), 
        (51, 'Обнаружено клиническое проявление симптомов психотравмы (III звено)'), 
    ),
    'ua' : (
        ( 3, 'Стан цілком нормальний'), 
        (17, 'Виявлено симптоми психотравми, які потребують психологічного супроводження фахівців травмофокусуючого напрямку'), 
        (34, 'Виявлено субклінічно проявлені симптоми психотравми (ІІ ланка)'), 
        (51, 'Виявлено клінічну проявленість симптомів психотравми (ІІІ ланка)'), 
    ),
}
...
def answer(bot, message, command, data=None, logger=None, question=None, **kw):
    ...
    if question == 17 or (IsDebug and question > 0 and question%10 == 0):
    ...
    if is_run:
        ...
        storage.set(name, '%s.R' % _TEST_NAME, _results['value'])
