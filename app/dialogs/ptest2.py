...
_CONCLUSIONS = {
    'ru' : (
        ( 2, 'Выявлена гиперэмотивность, плохое самопонимание или неоткровенность при обследовании'), 
        ( 4, 'Вы гармоничная личность'), 
        ( 7, 'Выявлены акцентуированные черты, умеренная выраженность'), 
        ( 9, 'Выявлена эмоциональная напряжённость, затруднённая адаптация, избыточная выраженность'), 
    ),
    'ua' : (
        ( 2, 'Виявлено гіперемотивність, погане само розуміння чи нещирість при обстеженні'), 
        ( 4, 'Ви гармонійна особистість'), 
        ( 7, 'Виявлено акцентуйовані риси, помірна вираженість'), 
        ( 9, 'Виявлено емоційну напруженість, утруднена адаптація, надлишкова вираженість'), 
    ),
}
...
def answer(bot, message, command, data=None, logger=None, question=None, **kw):
    ...
    if question == 91 or (IsDebug and question > 0 and question%10 == 0):
    ...
    if question > 61 and 'query_id' in kw:
    ...
    if is_run:
        ...
        for p in sorted([x for x in _RESULTS[lang].keys()]):
            if p in _results:
                storage.set(name, '%s.R%s' % (_TEST_NAME, p.split('.')[0]), _results[p])
