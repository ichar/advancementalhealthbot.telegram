...
def answer(bot, message, command, data=None, logger=None, question=None, **kw):
    ...
    if question == 84 or (IsDebug and question > 0 and question%10 == 0):
    ...
    if is_run:
        ...
        for p in sorted([x for x in _CONCLUSIONS[lang].keys()]):
            if p in _results:
                storage.set(name, '%s.RC%s' % (_TEST_NAME, p), _results[p][0])
